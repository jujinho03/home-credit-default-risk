...
수정 전 3위였던 `ORGANIZATION_TYPE_Realtor`는 표준화 효과 기준 93위, 6위였던 `ORGANIZATION_TYPE_Transport: type 3`은 88위로 내려가 희소 조직유형 중심 해석을 철회했습니다. 반면 `EXT_SOURCE_3`과 `EXT_SOURCE_2`는 두 검증 방법 모두 Top 10에 포함돼 외부 신용점수 계열이 일관된 상위 신호임을 확인했습니다. `AMT_CREDIT`과 `AMT_GOODS_PRICE`도 상위에 남았지만 서로 강하게 연관된 변수이므로, 개별 계수의 부호를 원인으로 해석하지 않고 **대출 규모와 관련된 묶음 신호**로만 해석했습니다. `BASEMENTAREA`·`LIVINGAREA` 등 건물정보 변수도 상위에 나타나지만, 이들은 결측률이 높고 `AVG`·`MEDI` 형태로 거의 동일한 정보가 반복되는 변수군이므로 개별 변수의 인과적 해석보다 **건물정보의 결측·기재 패턴이라는 묶음 신호**로 제한해 해석했습니다. 이 과정은 결과를 맹신하지 않고 기준을 맞춰 다시 검증한 프로젝트의 핵심 수정 사항입니다.
...
- RBF SVM의 표본 제한과 비교 조건은 §7 설명을 함께 확인해야 합니다.
